...
- Phase 4 complete: NEWS 0.2.10 entry added covering scheduler arg + OS-native backends + heartbeat logging + deprecation warning + pre-render fix + new mc_schedule.R module. Version bumped 0.2.9 → 0.2.10. jsonlite confirmed in Imports. Full suite 364 pass / 0 fail. Lint clean.
- Ready for `/gh-pr-push` and `/gh-pr-merge` (NEWS finalization, tag, post-merge CI watch). Then `/planning-archive`.
